...
\textbf{(i)}~FC-VQCs achieve 90--96\% of the $R^2$ of attention-based VQCs while using 40--50\% fewer parameters, and consistently outperform equal-capacity MLPs (mean $R^2{=}0.829$ vs.\ MLP$_{720}$'s $0.758$ on Boston Housing, 3-seed average);
...
\begin{table}[t]
\centering
\caption{Test $R^2$ on regression datasets. Quantum models: 3-seed mean $\pm$ std. Classical baselines: deterministic. Best quantum mean per column in \textbf{bold}. MLP$_{720}$ matches FC-VQC's parameter count. FC-VQC uses dataset-specific qubit layouts ($15\mathrm{t}5\mathrm{t}1$ for Boston's 13 features; $8\mathrm{t}3\mathrm{t}1$ for 8-feature datasets).}
...
\midrule
CatBoost & .861 & \textbf{.857} & \textbf{.933} & -- \\
XGBoost & .856 & .853 & .904 & -- \\
KRR (RBF) & .671 & .783 & .779 & -- \\
SVR (RBF) & .548 & .788 & .608 & -- \\
\midrule
MLP$_{\text{PM}}$ & .769 & .793 & .843 & 218 \\
MLP$_{720}$ & .758 & -- & -- & 721 \\
\midrule
...
\begin{table}[t]
\centering
\caption{Test accuracy on classification datasets. Quantum models: 3-seed mean $\pm$ std. Best quantum mean per column in \textbf{bold}.}
...
\midrule
XGBoost & \textbf{.671} & \textbf{.695} & -- \\
CatBoost & .667 & .675 & -- \\
\midrule
MLP$_{\text{PM}}$ & .604 & .573 & 198 \\
\midrule
...
FC-VQC achieves a 3-seed mean $R^2{=}0.829$ on Boston Housing with only 486--720 parameters.
While this trails CatBoost ($0.861$) and XGBoost ($0.856$), it substantially outperforms MLP$_{720}$ ($R^2{=}0.758$)---an MLP with the exact same parameter count---confirming that VQC's inductive bias provides genuine representational benefit beyond mere parameter count.
Notably, MLP$_{720}$ performs \emph{worse} than the smaller MLP$_{\text{PM}}$ ($0.769$), indicating that classical networks overfit at this dataset size when given more parameters, while FC-VQC does not.
...
FC-VQC reaches 90--96\% of QT's accuracy with 40--50\% fewer parameters---a trade-off that favors FC-VQC on parameter-constrained NISQ hardware.
...
The most notable finding is that \textbf{removing self-attention from FQT improves regression accuracy on Boston Housing} ($R^2$: $0.799 \!\to\! 0.894$, single seed).
...
FC-VQCs consistently outperform equal-capacity MLPs (mean $R^2{=}0.829$ vs.\ $0.758$ on Boston with ${\sim}$720 parameters), confirming a genuine VQC inductive bias advantage.
