...
\bottomrule
\multicolumn{6}{l}{\scriptsize $^\dagger$Wine R+W: single seed (ablation seeds 123, 7 pending).}
\end{tabular}
\vspace{-10pt}
\end{table}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{On Boston Housing} ($n{=}506$, the smallest dataset), removing attention \emph{improves} performance ($0.705 \to 0.851$, 3-seed mean) because the extra parameters in the attention block cause overfitting on the small training set. The Type~4 FFN alone provides sufficient mixing with far fewer parameters.
