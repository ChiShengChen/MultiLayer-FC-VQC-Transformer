...
% TODO: Add noise results after experiments complete.
...
Based on our findings: (i)~\textbf{default to FC-VQC} with depth-3 blocks for tabular regression; (ii)~use \textbf{ResNet-VQC} when training stability is needed; (iii)~\textbf{avoid multi-head quantum attention} on small tabular data; (iv)~add \textbf{LayerNorm to FQT} for classification tasks.
...
Quantum self-attention, despite its conceptual appeal, does not consistently improve performance on small tabular data and can even hurt it.
...
These results provide actionable guidance for the quantum ML community: FC-VQCs should be the default architecture for tabular tasks on near-term hardware, with ResNet-VQC as a robust alternative.
...
Future directions include validation on real quantum hardware, scaling to larger qubit counts, and exploring quantum attention on sequential tasks where the inductive bias of self-attention may be better matched to the data structure.
