...
\textbf{(i)}~FC-VQCs can outperform gradient-boosted trees on small datasets (e.g.\ $R^2{=}0.889$ vs.\ CatBoost's $0.861$ on Boston Housing with only 720 parameters), and consistently outperform parameter-matched MLPs across all datasets;
\textbf{(ii)}~FC-VQC's Type~4 inter-block connectivity provides partial cross-token mixing that approximates the role of attention---explicit quantum self-attention yields only marginal gains on most datasets while significantly increasing parameter count, and can even degrade performance on small datasets due to overfitting;
...
\begin{itemize}[leftmargin=*]
    ...
    \item Evidence that \textbf{FC-VQCs are the most parameter-efficient} quantum architecture, outperforming parameter-matched MLPs on all datasets and surpassing gradient-boosted trees on the smallest dataset ($n{=}506$) with only ${\sim}$700 parameters. On larger datasets, FC-VQCs trail tree-based methods but remain competitive with ${\sim}$500 parameters versus ${\sim}$13{,}000 for comparable MLPs.
...
On Boston Housing ($n{=}506$), FC-VQC$_{15\mathrm{t}5\mathrm{t}1}$ achieves $R^2{=}0.889$ with only 720 parameters, outperforming both XGBoost ($0.856$) and CatBoost ($0.861$).
On the larger CA Housing ($n{=}20{,}640$) and Concrete ($n{=}1{,}030$) datasets, FC-VQCs trail gradient-boosted trees ($R^2{=}0.752$ vs.\ CatBoost's $0.857$ on CA Housing; $0.756$ vs.\ $0.933$ on Concrete), indicating that the parameter advantage narrows as dataset size grows and tree-based methods can leverage more data.
To control for parameter count, we also train an MLP with 721 parameters (MLP$_{720}$), matching FC-VQC exactly.
This MLP achieves only $R^2{=}0.758$ on Boston---\emph{lower} than the 218-parameter MLP ($0.769$)---indicating that simply adding parameters to a classical network causes overfitting at this dataset size.
In contrast, FC-VQC uses the same 720 parameters to reach $R^2{=}0.889$, confirming that the quantum circuit's inductive bias provides genuine representational benefit that classical networks cannot match at equal capacity.
...
Notably, this improvement is specific to Boston Housing ($n{=}506$), the smallest dataset; on the other four datasets, removing attention slightly degrades performance.
...
\subsection{Statistical Validation Across Seeds}
\label{sec:results-multiseed}

To assess the robustness of our findings, we repeat all four key quantum models across three random seeds (varying both data split and parameter initialization) on all five datasets.
\cref{tab:multiseed-regression,tab:multiseed-classification} report mean $\pm$ std.

\begin{table}[t]
\centering
\caption{Test $R^2$ across 3 random seeds (mean $\pm$ std). Best mean per column in \textbf{bold}.}
...
\midrule
FC-VQC & $\mathbf{.829}{\scriptstyle\pm.042}$ & $.750{\scriptstyle\pm.001}$ & $.774{\scriptstyle\pm.021}$ & 486--720 \\
ResNet & $.775{\scriptstyle\pm.040}$ & $.783{\scriptstyle\pm.004}$ & $.819{\scriptstyle\pm.028}$ & 486--720 \\
QT & $.742{\scriptstyle\pm.071}$ & $\mathbf{.807}{\scriptstyle\pm.001}$ & $\mathbf{.853}{\scriptstyle\pm.011}$ & 828--1380 \\
...
\begin{table}[t]
\centering
\caption{Test accuracy across 3 random seeds (mean $\pm$ std).}
\label{tab:multiseed-classification}
\small
\begin{tabular}{lrrr}
\toprule
Model & Wine-R & Wine-RW & \#p \\
\midrule
FC-VQC & $.592{\scriptstyle\pm.029}$ & $.567{\scriptstyle\pm.013}$ & 612 \\
ResNet & $.601{\scriptstyle\pm.019}$ & $.567{\scriptstyle\pm.011}$ & 570 \\
QT & $.565{\scriptstyle\pm.023}$ & $.567{\scriptstyle\pm.015}$ & 1098 \\
FQT & $\mathbf{.606}{\scriptstyle\pm.017}$ & $.565{\scriptstyle\pm.019}$ & 678 \\
\bottomrule
\end{tabular}
\vspace{-5pt}
\end{table}

The multi-seed results confirm the main findings while adding nuance.
FC-VQC achieves the highest mean $R^2$ on Boston ($0.829$) but with substantial variance ($\pm 0.042$), reflecting sensitivity to initialization on the smallest dataset ($n{=}506$).
On CA Housing and Concrete, QT is the best quantum model ($0.807$ and $0.853$) with low variance ($\pm 0.001$ and $\pm 0.011$), but at $2{\times}$--$3{\times}$ the parameter count of FC-VQC.
For classification, all models perform similarly (means within $0.04$), with FQT slightly ahead on Wine Red ($0.606 \pm 0.017$).

Notably, the variance on larger datasets (CA Housing, Wine R+W) is much smaller ($\pm 0.001$--$0.019$) than on Boston ($\pm 0.042$--$0.078$), confirming that the Boston results are the least stable and should be interpreted with caution.
...
Our central finding concerns the \textbf{parameter-efficiency trade-off of attention}: FC-VQC's Type~4 inter-block connectivity provides partial cross-token mixing that, combined with VQC nonlinearity, captures most of the benefit of explicit attention at a fraction of the parameter cost.
Explicit attention yields marginal gains on most datasets but can hurt on small training sets ($n{=}506$) where the extra parameters cause overfitting.
FC-VQCs surpass gradient-boosted trees on the smallest dataset and consistently outperform parameter-matched MLPs, using only ${\sim}$500--720 parameters versus $2{\times}$--$6{\times}$ more for attention-based VQCs.
On larger datasets, FC-VQCs trail tree-based methods, suggesting the advantage is strongest in the low-data regime most relevant to near-term quantum hardware.

Additional findings reinforce the practical value of simpler architectures: expressibility saturates at depth~${\approx}\,3$; FQT degrades gracefully under depolarizing noise while QT collapses due to softmax amplification; and moderate noise can act as implicit regularization.
