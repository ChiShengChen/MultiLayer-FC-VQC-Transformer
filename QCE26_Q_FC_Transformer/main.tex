...
\cref{fig:architectures} provides a visual overview of three key architectures.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/architectures_3panel.png}
\vspace{-15pt}
\caption{Three model architectures compared. (a)~FC-VQC with decreasing qubit count per block. (b)~ResNet-VQC with dashed skip connections; the dimension-mismatched branch uses a linear projection $W \cdot \mathbf{z}_2$. (c)~FQT with fully quantum attention (VQC$_Q$, VQC$_K$, VQC$_V$) and no LayerNorm by default.}
