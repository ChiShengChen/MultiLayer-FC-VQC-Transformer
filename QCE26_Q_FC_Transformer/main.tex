...
\begin{table}[t]
\centering
\caption{Test $R^2$ on regression datasets. All results: 3-seed mean $\pm$ std. Best overall mean per column in \textbf{bold}. Tree-based \#params estimated as leaf values + split parameters (500 trees, depth 6).\footnotemark{} $n_{\text{tr}}$: dual coefficients scale with training set size. FC-VQC uses dataset-specific qubit layouts.}
