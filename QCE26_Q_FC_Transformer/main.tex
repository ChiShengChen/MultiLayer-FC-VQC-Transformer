...
\textbf{(ii)}~quantum self-attention provides no consistent benefit on small-scale tabular data---ablations show that removing attention can \emph{improve} performance;
...
\begin{itemize}[leftmargin=*]
    ...
    \item A \textbf{negative result on quantum attention}: self-attention does not consistently help on small tabular data, and removing it can improve performance---an important finding for practitioners.
    \item Practical guidance: expressibility saturates at depth~${\approx}\,3$, LayerNorm benefits fully quantum architectures on classification, and residual connections offer a reliable middle ground between FC-VQC and transformer complexity.
\end{itemize}
...
These results suggest that multi-head quantum attention is not a reliable way to improve performance on small tabular datasets.
...
On other datasets the effect is smaller or reversed, but the overall message is clear: quantum self-attention does not provide a consistent benefit on tabular data at this scale.
...
\paragraph{When might quantum attention help?}
Our negative result is specific to \emph{tabular data with few features} (${\leq}$13 dimensions, ${\leq}$5 tokens).
In this regime, the token count is small enough that a single transpose operation achieves full mixing.
Quantum attention may become beneficial when: (i)~the number of tokens $T$ is large enough that all-to-all FFN mixing becomes impractical or parameter-heavy; (ii)~the data has sequential or positional structure (e.g., time series) where \emph{selective} attention over certain positions outperforms uniform mixing; or (iii)~the feature interactions are sparse and data-dependent, requiring the input-adaptive gating that softmax attention provides.
...
Our central finding is that \textbf{simpler is better}: multi-layer FC-VQCs achieve the strongest accuracy-to-parameter ratio, matching gradient-boosted trees on regression tasks with ${\sim}$700 parameters.
Quantum self-attention, despite its conceptual appeal, does not consistently improve performance on small tabular data and can even hurt it---a structural consequence of the Type~4 FFN already providing all-to-all token mixing.
Expressibility analysis confirms that shallow circuits (depth~${\approx}\,3$) already saturate the accessible Hilbert space, explaining why deep or complex architectures provide diminishing returns.
Noise experiments reveal that FQT degrades gracefully under depolarizing noise while QT collapses, and that moderate noise can act as implicit regularization.

These results provide actionable guidance for the quantum ML community: FC-VQCs should be the default architecture for tabular tasks on near-term hardware, with ResNet-VQC as a robust alternative and FQT preferred over QT when hardware noise is a concern.
Transformer-style VQCs may find their niche on structured, sequential, or high-dimensional data where attention mechanisms have proven beneficial in the classical setting, but this remains a direction for future work.
Future directions include validation on real quantum hardware, broader noise studies across datasets, and exploring quantum attention on sequential tasks where the inductive bias of self-attention may be better matched to the data structure.
