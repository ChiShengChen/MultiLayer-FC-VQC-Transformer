...
\IEEEauthorblockA{Beth Israel Deaconess Medical Center \& Harvard Medical School, Boston, MA, USA\\
\href{mailto:cchen31@bidmc.harvard.edu}{cchen31@bidmc.harvard.edu}}
}
\else
...
\textbf{(i)}~FC-VQCs achieve competitive or superior regression accuracy to gradient-boosted trees with \emph{orders-of-magnitude fewer parameters} (e.g.\ $R^2{=}0.889$ on Boston Housing with only 720 parameters);
\textbf{(ii)}~FC-VQC's fully-connected inter-block connectivity already implements an \emph{implicit uniform attention} mechanism---adding explicit quantum self-attention on top is structurally redundant and can even hurt performance;
...
\textbf{(v)}~FQT degrades gracefully under depolarizing noise while QT collapses, and moderate noise can act as implicit regularization.
These results provide practical architectural guidance for deploying VQCs on near-term quantum hardware.
\end{abstract}
...
\begin{itemize}[leftmargin=*]
    ...
    \item Evidence that \textbf{FC-VQCs are the most parameter-efficient} architecture, achieving regression accuracy competitive with gradient-boosted trees using ${\sim}$700 parameters versus ${\sim}$13{,}000 for equivalently accurate MLPs.
    \item A formal analysis showing that \textbf{FC-VQC's Type~4 connectivity is structurally equivalent to uniform attention} with nonlinear VQC mixing, explaining why adding explicit self-attention is redundant and can degrade performance on small tabular data.
...
Recent work has highlighted that deep learning often underperforms tree ensembles on tabular data, making them an essential baseline for any quantum ML study claiming practical relevance.

%===================================================================
\section{Methods}
\label{sec:methods}

We compare four VQC architecture families, all built on the \texttt{StronglyEntanglingLayers} ansatz from PennyLane as the base quantum circuit block.
...
\cref{tab:main-regression,tab:main-classification} present the main comparison across all architectures and baselines.
...
\midrule
FC-VQC$_{15\mathrm{t}5\mathrm{t}1}$ & \textbf{0.889} & -- & -- & 720 \\
FC-VQC$_{8\mathrm{t}3\mathrm{t}1}$ & -- & 0.752 & 0.756 & 486 \\
ResNet-VQC & 0.831 & 0.785 & 0.858 & 486--720 \\
QT ($H{=}1$) & 0.781 & 0.807 & 0.838 & 1{,}380 \\
QT ($H{=}3$) & 0.834 & 0.814 & 0.837 & 4{,}350 \\
FQT ($H{=}1$) & 0.799 & 0.786 & 0.792 & 855 \\
FQT ($H{=}3$) & 0.808 & 0.806 & 0.757 & 2{,}745 \\
\bottomrule
\end{tabular}
\vspace{-5pt}
\end{table}
...
\begin{table}[t]
\centering
\caption{Test accuracy and macro-F1 on classification datasets. Best per column in \textbf{bold}.}
\label{tab:main-classification}
\small
\begin{tabular}{lrrrrr}
\toprule
Model & \multicolumn{2}{c}{Wine (Red)} & \multicolumn{2}{c}{Wine (R+W)} & \#p \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
 & Acc & F1 & Acc & F1 & \\
\midrule
XGBoost & \textbf{.671} & \textbf{.333} & \textbf{.695} & \textbf{.478} & -- \\
CatBoost & .667 & .330 & .675 & .422 & -- \\
\midrule
MLP$_{\text{PM}}$ & .604 & .283 & .573 & .287 & 198 \\
\midrule
FC-VQC & .596 & .265 & .585 & .264 & 612 \\
ResNet & .608 & .293 & .579 & .289 & 570 \\
QT ($H{=}3$) & .604 & .306 & .595 & .300 & 3{,}258 \\
FQT ($H{=}1$) & .608 & .293 & .592 & .294 & 678 \\
\bottomrule
\end{tabular}
\vspace{-10pt}
\end{table}

On regression tasks, the most striking finding is the \textbf{parameter efficiency of FC-VQCs}.
On Boston Housing, FC-VQC$_{15\mathrm{t}5\mathrm{t}1}$ achieves $R^2{=}0.889$ with only 720 parameters, outperforming both XGBoost ($0.856$) and CatBoost ($0.861$).
A parameter-matched MLP with 218 parameters reaches only $R^2{=}0.769$, and a full MLP requires ${\sim}$13{,}000 parameters to approach comparable accuracy---a roughly $18{\times}$ parameter disadvantage.

Quantum Transformer variants (QT, FQT) achieve respectable scores on CA~Housing ($R^2{\leq}0.814$) and Concrete ($R^2{\leq}0.869$), but at significantly higher parameter counts (1{,}098--4{,}350) than FC-VQCs (486--720), and they do not consistently outperform the simpler architectures.
ResNet-VQC provides a reliable middle ground: competitive accuracy with the same parameter budget as FC-VQC.

On classification (\cref{tab:main-classification}), all quantum models trail tree-based methods by 6--10\% in accuracy.
This is expected: tabular classification is a well-known strength of gradient-boosted ensembles.
Among quantum models, FQT achieves the highest accuracy ($0.608$ on Wine Red), matching ResNet-VQC.
...
If FC-VQC's Type~4 FFN already provides uniform all-to-all mixing, then multi-head attention adds redundant data-dependent selection on top of it---explaining why extra heads increase parameters without consistent gains when $T$ is small.

\subsection{Architecture Ablation: Explicit Attention Is Redundant}
\label{sec:results-ablation}

\cref{tab:ablation} presents ablation experiments removing or modifying key architectural components.
These results are best understood through the lens of FC-VQC as implicit attention (formalized in \cref{sec:discussion}).

\begin{table}[t]
\centering
\caption{Architecture ablation. Regression: test $R^2$; classification: test accuracy. $-$attn removes self-attention; T3 uses circular-shift (Type~3) FFN; +LN adds LayerNorm to FQT.}
\label{tab:ablation}
\small
\begin{tabular}{lrrrrr}
\toprule
...
\midrule
QT (default) & .781 & .807 & .838 & .588 & .587 \\
\quad $-$attn & .796 & .800 & .797 & .608 & .581 \\
\quad T3 FFN & .778 & .808 & .820 & .613 & .590 \\
\midrule
FQT (default) & .799 & .786 & .792 & .608 & .592 \\
\quad $-$attn & \textbf{.894} & .775 & .765 & .600 & .578 \\
\quad T3 FFN & .885 & .798 & .818 & .592 & .593 \\
\quad +LN & .810 & \textbf{.832} & .756 & \textbf{.629} & \textbf{.597} \\
\bottomrule
\end{tabular}
\vspace{-10pt}
\end{table}

The most notable finding is that \textbf{removing self-attention from FQT improves regression accuracy on Boston Housing} ($R^2$: $0.799 \!\to\! 0.894$).
This is the highest $R^2$ achieved by any quantum model in our study, and it comes from a \emph{simpler} architecture.
A similar pattern appears with Type~3 FFN ($0.885$).
On other datasets the effect is smaller or reversed.
These results are consistent with the hypothesis that FC-VQC's Type~4 connectivity already provides sufficient inter-token mixing---adding explicit attention introduces redundant parameters that hinder optimization without improving representational capacity.
...
\cref{tab:expressibility} and \cref{fig:expressibility} show that expressibility saturates at depth~$\approx\!3$, with KL divergence dropping from $0.20$ (depth~1) to $0.003$ (depth~3) and plateauing thereafter.
The classical linear projection baseline is roughly three orders of magnitude less expressive ($\mathrm{KL}{=}1.91$), confirming that even shallow VQC layers access a significantly richer region of Hilbert space.
...
Second, and surprisingly, \textbf{moderate noise can improve performance}: FQT achieves $R^2{=}0.898$ at $p_d{=}0.005$, higher than its noiseless baseline ($0.799$).
This suggests that depolarizing noise acts as a form of implicit regularization, analogous to dropout in classical networks, preventing overfitting in the small-data regime ($n{=}506$).
...
On regression tasks with moderate dimensionality, FC-VQCs can match or exceed strong classical baselines \emph{while using orders of magnitude fewer parameters}.
...
\paragraph{FC-VQC as implicit uniform attention}

Perhaps our most surprising finding is that quantum self-attention does not reliably improve performance.
The ablation in \cref{tab:ablation} shows that FQT \emph{without} attention achieves the highest $R^2$ on Boston Housing ($0.894$).
We argue this is not merely a data-scale artifact but a \emph{structural consequence}: the Type~4 FFN already implements a form of attention.

To see this, consider the classical self-attention output for token $i$:
\begin{equation}
    \mathbf{o}_i = \sum_{j=1}^{T} \alpha_{ij}\, \mathbf{v}_j, \quad
    \alpha_{ij} = \frac{\exp(\mathbf{q}_i \!\cdot\! \mathbf{k}_j / \sqrt{d})}
    {\sum_{j'} \exp(\mathbf{q}_i \!\cdot\! \mathbf{k}_{j'} / \sqrt{d})}.
    \label{eq:classical-attn}
\end{equation}
The weights $\alpha_{ij}$ are data-dependent and input-adaptive.
Now consider what Type~4 FFN does.
It stacks the $T$ token embeddings $\mathbf{m}_1, \dots, \mathbf{m}_T \in \mathbb{R}^3$ into a matrix $\mathbf{M} \in \mathbb{R}^{T \times 3}$, transposes to $\mathbf{M}^\top \in \mathbb{R}^{3 \times T}$, and re-chunks into $T$ groups of~3.
...
\end{equation}
where $\pi$ is the deterministic index mapping from the transpose-and-flatten operation.
Crucially, each $\tilde{\mathbf{m}}_i$ contains exactly one element from each of $T$ different tokens.
This is equivalent to applying \cref{eq:classical-attn} with \textbf{uniform attention weights} $\alpha_{ij} = 1/T$, followed by a nonlinear VQC transformation $f_\theta$:
\begin{equation}
    \mathbf{o}_i^{\text{FC}} = f_\theta\!\left(\frac{1}{T}\sum_{j=1}^{T} \mathbf{P}_{ij}\, \mathbf{m}_j\right),
    \label{eq:fc-as-attn}
\end{equation}
where $\mathbf{P}_{ij}$ is a fixed permutation matrix selecting one feature dimension per token, and $f_\theta$ is the parameterized VQC.

The key difference between \cref{eq:fc-as-attn} and \cref{eq:classical-attn} is:
\begin{itemize}[leftmargin=*]
    \item \textbf{Classical attention}: data-dependent weights $\alpha_{ij}$ + linear value projection.
    \item \textbf{FC-VQC Type~4}: uniform weights (fixed permutation) + \emph{nonlinear} VQC processing.
\end{itemize}
The VQC's nonlinearity compensates for the lack of data-dependent weighting: while attention \emph{selects} which tokens to mix, the VQC \emph{learns a nonlinear function} over the mixed representation.
When $T$ is small (${\leq}5$ in our experiments), the selection mechanism provides negligible benefit because the VQC can process all tokens simultaneously.
In this regime, \textbf{FC-VQC is a uniform-attention transformer where the VQC replaces softmax selection with learned nonlinear mixing}.

This perspective explains why adding explicit attention on top of Type~4 FFN is redundant---the model already performs attention-like all-to-all token mixing, and stacking a second mixing stage only adds parameters without representational benefit.

This analysis yields three concrete predictions, all confirmed by our experiments:
\begin{enumerate}[leftmargin=*]
    \item \textbf{Removing attention from FQT should not degrade---and may improve---performance} when the FFN already uses Type~4 connectivity.
    This is confirmed: FQT$-$attn achieves $R^2{=}0.894$ on Boston vs.\ $0.799$ for default FQT (\cref{tab:ablation}).
    \item \textbf{Switching from Type~4 to Type~3 (circular-shift) FFN should have a larger impact than removing attention}, because Type~3 eliminates all-to-all mixing entirely.
    This is partially confirmed: FQT with Type~3 FFN ($0.885$) performs closer to FQT$-$attn ($0.894$) than to default FQT ($0.799$), suggesting that it is the \emph{FFN connectivity}, not attention, that drives performance.
    \item \textbf{Multi-head attention should not help} when single-head already provides sufficient mixing on small token counts ($T \leq 5$).
    This is confirmed: increasing $H$ from 1 to 3 yields inconsistent gains despite $2{\times}$--$3{\times}$ more parameters (\cref{tab:multihead}).
\end{enumerate}

\paragraph{When does data-dependent attention outperform uniform mixing?}
Our finding that FC-VQC's implicit uniform attention suffices is specific to \emph{tabular data with few features} (${\leq}$13 dimensions, ${\leq}$5 tokens).
In this regime, $T$ is small enough that uniform mixing covers all inter-token interactions, and VQC nonlinearity compensates for the lack of selective weighting.
Data-dependent attention (explicit QT/FQT) may become beneficial when: (i)~$T$ is large enough that uniform mixing becomes parameter-heavy or dilutes relevant signals; (ii)~the data has sequential or positional structure (e.g., time series) where \emph{selective} attention over certain positions outperforms uniform mixing; or (iii)~the feature interactions are sparse and data-dependent, requiring the input-adaptive gating that softmax provides.
...
\paragraph{Noise as implicit regularization}
The noise robustness results (\cref{tab:noise}) reveal that moderate depolarizing noise ($p_d{=}0.005$) can \emph{improve} FQT performance beyond its noiseless baseline.
This regularization effect is consistent with the small training set size ($n{=}506$) and suggests that VQCs on NISQ hardware may benefit from---rather than be harmed by---realistic noise levels, provided the architecture avoids classical amplification bottlenecks (e.g., softmax on noisy inputs).
...
We presented a systematic comparison of four VQC architectures---FC-VQC, ResNet-VQC, QT, and FQT---on five tabular benchmarks.
Our central finding is that \textbf{FC-VQC already performs attention}: its Type~4 fully-connected inter-block connectivity implements an implicit uniform-attention mechanism where VQC nonlinearity replaces softmax selection.
This explains both the parameter efficiency of FC-VQCs (matching gradient-boosted trees with ${\sim}$700 parameters) and why adding explicit quantum self-attention is redundant---the architecture is already mixing all tokens, and stacking a second mixing stage only adds parameters without representational benefit.

This perspective reframes the architectural design question: rather than asking \emph{``how do we add attention to VQCs?''}, the community should ask \emph{``when does data-dependent attention selection outperform uniform nonlinear mixing?''} Our experiments suggest the answer is: not on small tabular data ($T \leq 5$ tokens), where FC-VQC's uniform mixing suffices.

Additional findings reinforce this view: expressibility saturates at depth~${\approx}\,3$, confirming that shallow VQCs are already expressive enough; FQT degrades gracefully under depolarizing noise while QT collapses, showing that classical attention amplifies noise through softmax; and moderate noise can act as implicit regularization.
...
Transformer-style VQCs may find their niche on sequential or high-dimensional data where $T$ is large enough that data-dependent attention selection outperforms uniform mixing---testing this hypothesis is a key direction for future work.
