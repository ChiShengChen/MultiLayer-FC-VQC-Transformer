...
Several recent works have explored quantum analogues of the transformer architecture, ranging from quantum self-attention mechanisms operating on amplitude-encoded data to fully quantum multi-head attention with parameterized entangling gates.
Our work differs in providing a controlled comparison against simpler VQC baselines (FC-VQC, ResNet-VQC) on identical benchmarks, rather than proposing a new transformer variant in isolation.
...
\paragraph{Why attention is redundant under FC-VQC connectivity}
...
We argue this is not merely a data-scale artifact but a \emph{structural consequence} of the FC-VQC connectivity design.

Recall that the Type~4 (fully-connected) FFN operates by stacking all $T$ token embeddings into a matrix $\mathbf{M} \in \mathbb{R}^{T \times 3}$, transposing it to $\mathbf{M}^\top \in \mathbb{R}^{3 \times T}$, and feeding each row into a 3-qubit VQC.
After this operation, each output VQC has already seen information from \emph{every token}.
This transpose-and-process step is precisely the same structural operation that FQT's quantum attention performs (\cref{sec:fqt})---except that the FFN version applies it with 3-qubit circuits (one per feature dimension) while FQT attention uses $T$-qubit circuits (one per feature dimension).

In other words, \textbf{FC-VQC's Type~4 FFN already implements a form of all-to-all inter-token mixing} that is functionally equivalent to what attention is supposed to provide.
Adding an explicit attention layer on top of this is redundant: the model must learn two successive mixing stages where one suffices.
The extra parameters in the attention block ($3 \times T$ VQC parameter sets for Q/K/V in QT, or $3$ additional $T$-qubit VQCs in FQT) do not increase the representational capacity enough to justify the optimization burden they introduce.
