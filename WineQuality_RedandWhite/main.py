...
from models import MLPClassifier, SingleVQC_12t6, FullyConnectedVQCs_12t8t6, FullyConnectedVQCs_24t8t6, FullyConnectedVQCs_36t12t8t6, FullyConnectedVQCs_48t16t6, FullyConnectedVQCs_60t20t7t6, CatBoostClassifierModel, XGBoostClassifierModel
...
CSV_PATH = os.path.join(DATA_DIR, "wine-quality-white-and-red.csv")
...
def main(vqc_depth: int, hidden_layers: int):
    ...
    # 3) Model selection
    # ----------------------------
    # model = SingleVQC_12t6(layers=hidden_layers, depth=vqc_depth).to(device)
    model = FullyConnectedVQCs_12t8t6(layers=hidden_layers, depth=vqc_depth).to(device)
    # model = FullyConnectedVQCs_24t8t6(layers=hidden_layers, depth=vqc_depth).to(device)
    # model = FullyConnectedVQCs_36t12t8t6(layers=hidden_layers, depth=vqc_depth).to(device)
    # model = FullyConnectedVQCs_48t16t6(layers=hidden_layers, depth=vqc_depth).to(device)    
    # model = FullyConnectedVQCs_60t20t7t6(layers=hidden_layers, depth=vqc_depth).to(device)
