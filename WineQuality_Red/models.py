...
class SingleVQC_11t6(nn.Module):
    ...
    def __init__(self, layers, depth):
        ...
        self.dev_Q11 = qml.device("default.qubit", wires=11, shots=shots)

        # θ_list[t][l][d] -> Parameter[vqc_depth, n_qubits, 3]
        self.quantum_net_Q11 = qml.QNode(q_NtoN_Strong_function,  self.dev_Q11, interface="torch")  

        self.theta_Q11 = nn.ParameterList([
    ...
    def forward(self, x_):
        ...
        for i in range(self.layers):
            H = torch.stack(self.quantum_net_Q11(H,self.theta_Q11[i], 11)).T.float()

        return H[:,3:9] * self.multiplier
...
class FullyConnectedVQCs_12t8t6(nn.Module):
    ...
    def forward(self, x_):
        ...
        for i in range (self.layers):
            H = self._quantum_layer_Q3_3to3(H,thetas_layer=self.theta_Q3_list[i+2],n_qubits=3,mode="multiple")  # (B,12)
...
class FullyConnectedVQCs_22t8t6(nn.Module):
...
class FullyConnectedVQCs_33t12t8t6(nn.Module):
...
class FullyConnectedVQCs_44t15t10t6(nn.Module):
    ...
    def __init__(self, layers, depth):
        ...
        # Devices (one is fine; we use same dev for all QNodes)
        self.dev_Q3 = qml.device("default.qubit", wires=3, shots=shots)
        self.dev_Q10 = qml.device("default.qubit", wires=10, shots=shots)

        # θ_list[t][l][d] -> Parameter[vqc_depth, n_qubits, 3]
        self.quantum_net_Q3_3to3 = qml.QNode(q_NtoN_Strong_function,  self.dev_Q3, interface="torch")  
        self.quantum_net_Q3_3to2 = qml.QNode(q_Nto2_Strong_function,  self.dev_Q3, interface="torch")  
        self.quantum_net_Q3_3to1 = qml.QNode(q_Nto1_Strong_function,  self.dev_Q3, interface="torch")  
        self.quantum_net_Q10_10to10 = qml.QNode(q_NtoN_Strong_function,  self.dev_Q10, interface="torch")  
        ...
        self.theta_Q10 = nn.Parameter(0.01 * torch.randn(self.vqc_depth, 10, 3))
    ...
    def forward(self, x_):
        ...
        batch_size = H.size(0)
        zeros_col = torch.zeros(batch_size, 1, device=H.device, dtype=H.dtype)    
        # Adding a zero value to make it 45 values 
        H = torch.cat([zeros_col, H], dim=1)  # (batch, 45)  

        # Layer 0: Q3 VQCs 45 to 45
        H = self._quantum_layer_Q3_3to3(H,thetas_layer=self.theta_Q3_list[2],n_qubits=3,mode="first")  # (B,45)
        for i in range (self.layers):
            H = self._quantum_layer_Q3_3to3(H,thetas_layer=self.theta_Q3_list[i+3],n_qubits=3,mode="multiple")  # (B,45)

        # Q3 45 to 15
        H = self._quantum_layer_Q3_3to1(H,thetas_layer=self.theta_Q3_list[1],n_qubits=3)  # (B,15) 

        # Q3 15 to 10
        H = self._quantum_layer_Q3_3to2(H,thetas_layer=self.theta_Q3_list[0],n_qubits=3)  # (B,10) 

        # Q10 10 to 6
        H = torch.stack(self.quantum_net_Q10_10to10(H, self.theta_Q10, 10)).T.float()
        H = H[:,2:8]
...
class FullyConnectedVQCs_44t15t10t8t6(nn.Module):
    ...
    def forward(self, x_):
        ...
        # Layer 0: Q3 VQCs 45 to 45
        H = self._quantum_layer_Q3_3to3(H,thetas_layer=self.theta_Q3_list[3],n_qubits=3,mode="first")  # (B,45)
        for i in range (self.layers):
            H = self._quantum_layer_Q3_3to3(H,thetas_layer=self.theta_Q3_list[i+4],n_qubits=3,mode="multiple")  # (B,45)

        # Q3 45 to 15
        H = self._quantum_layer_Q3_3to1(H,thetas_layer=self.theta_Q3_list[2],n_qubits=3)  # (B,15) 

        # Q3 15 to 10
        H = self._quantum_layer_Q3_3to2(H,thetas_layer=self.theta_Q3_list[1],n_qubits=3)  # (B,10) 

        # Adding two zero values to make it 12 values
        H = torch.cat([zeros_col, H, zeros_col], dim=1)  # (batch, 12)  
        # Q3 12 to 8
        H = self._quantum_layer_Q3_3to2(H,thetas_layer=self.theta_Q3_list[0],n_qubits=3)  # (B,8) 
...
class FullyConnectedVQCs_54t18t6(nn.Module):
    ...
    def forward(self, x_):
        ...
        # removing a value to make it 54 values 
        H = H[:, 0:54]
...
class FullyConnectedVQCs_54t18t6t6(nn.Module):
    ...
    def __init__(self, layers, depth):
        ...
        # self.quantum_net_Q3_3to2 = qml.QNode(q_Nto2_Strong_function,  self.dev_Q3, interface="torch")  
        self.quantum_net_Q3_3to1 = qml.QNode(q_Nto1_Strong_function,  self.dev_Q3, interface="torch")  
        self.quantum_net_Q6_6to6 = qml.QNode(q_NtoN_Strong_function,  self.dev_Q6, interface="torch")  
    ...
    def forward(self, x_):
        ...
        # Q6 6 to 6
        H = torch.stack(self.quantum_net_Q6_6to6(H, self.theta_Q6, 6)).T.float()

        return H * self.multiplier


class FullyConnectedVQCs_66t24t8t6(nn.Module):
    ...
    def forward(self, x_):
        ...
        # Layer 0: Q3 VQCs 66 to 66
        H = self._quantum_layer_Q3_3to3(H,thetas_layer=self.theta_Q3_list[2],n_qubits=3,mode="first")  # (B,66)
        for i in range (self.layers):
            H = self._quantum_layer_Q3_3to3(H,thetas_layer=self.theta_Q3_list[i+3],n_qubits=3,mode="multiple")  # (B,66)

        # Q3 66 to 22
        H = self._quantum_layer_Q3_3to1(H,thetas_layer=self.theta_Q3_list[1],n_qubits=3)  # (B,22) 

        # Adding zero values to make it 24 values 
        H = torch.cat([zeros_col, H, zeros_col], dim=1)  # (batch, 24)  
